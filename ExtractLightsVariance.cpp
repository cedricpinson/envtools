 ...
 */
void split_recursive(const satRegion& r, const uint n, std::vector<satRegion>& regions)
{
 ...
 */
void median_variance_cut(const SummedAreaTable& img, const uint n, std::vector<satRegion>& regions)
{
    regions.clear();
...
void outputJSON(const std::vector<light> &lights, uint height, uint width, uint imageAreaSize)
{
    ...
    for (std::vector<light>::const_iterator l = lights.begin(); l != lights.end() && i < lightNum; ++l) {
        ...
        // same as envremap util conversion
        const double lat = PI2 - PI * l->_centroidPosition._y / height;
        const double lon = TAU * l->_centroidPosition._x / width - PI;

        d._x =  sinf(lon) * cosf(lat);
        d._y =              sinf(lat);
        d._z = -cosf(lon) * cosf(lat);
...
int main(int argc, char** argv)
{
    ...
    median_variance_cut(lum_sat, 8, regions); // max 2^n cuts
